...
void DrawBitmap(uint8_t* buf,uint8_t x,uint8_t y,uint8_t width,uint8_t height,const uint8_t* bitmap)
{
    ...
    for(uint16_t u=0;u<height*width/8;++u)
    {
        ...
        {
            SetPixel(buf,x+_x,y+_y,( bitmap[u] & ( 1<< (7-c) )));
            ++_x;
        }
...
{
    DrawLine(buffer,left,top,right,top,color);
    DrawLine(buffer,right,top,right,bottom,color);
    DrawLine(buffer,right,bottom,left,bottom,color);
    DrawLine(buffer,left,bottom,left,top,color);
}
